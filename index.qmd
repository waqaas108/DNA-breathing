...
To do this, we checked all 6 of the YY1 cell lines in our dataset, evaluated their sequences with our model, and compared the performance with biophysical features and without. We were doing this using zero-nullification at inference, which means that in the test-set of the model, the evaluation with biophysical features would be subtracted by an evaluation with the biophysical features being forcibly replaced with a zero-array.
...
We acquired gcPBM sequences from Alexandrov Boian's lab, and then used their pyDNA-EPBD simulation package to generate bubble probabilities for the sequences. gcPBM as a method differs from ChIP-seq in key ways: it is a direct measurement of TF binding affinity, and it is performed in vitro. This means that the sequences are not subject to the same biases as ChIP-seq data, and the binding affinity is measured directly, rather than inferred from the binding strength of the TF. The sequences are also much shorter, at 36 base pairs, and the data is much more sparse, with only 3 TFs. However, the data is much more reliable, and the sequences are much more likely to be true binding sites. The affinity is a value between 0 and 1, with 1 being the highest affinity.

## [Phase 4](/phase4page_new.qmd)

## [Phase 4 (obsolete data)](/phase4page.qmd)

### Acquisition and processing of ChIP-seq sequences and bubble features

We used the ChIP-seq sequences available to us previously and then used pyDNA-EPBD to generate bubble data for these sequences. We then ran similar statistical tests to phase 3 to see if there was any correlation between bubble features and binding affinity. We also ran correlation tests between the average flipping feature value for a motif occurrence and the binding affinity of the sequence, and then correlations between the average flipping feature for individual base pair positions and the binding affinity of the sequence.

Using the results obtained, we used the 5 topmost correlated TFs as well as PAX5 and EBF1 and repeated our gcPBM analysis with ChIP-seq data.